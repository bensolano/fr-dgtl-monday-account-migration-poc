...
- **Endpoint:** `POST /api/v1/jobs` (`src/api/routers/jobs.py -> create_job`)
- **GCP Services:**
...
- **Method:** `JobService.execute_discovery_job(job_id)` (Engine logic injected via Dependency Injection / Protocols).
- **GCP Services:** **Secret Manager** (reads the source API key via `src.infrastructure.gcp.services` utility to instantiate `MondayClient`).

### Step 3: Fetching the Data

- **Class:** `DiscoveryService` (Satisfies `DiscovererInterface`)
...
- **Class:** `ClassificationService` (Satisfies `ClassifierInterface`)
...
- **Class:** `ReportService` (Satisfies `ReporterInterface`)
- **Method:** `generate_markdown_report(classified_inventory)`
- **GCP Services:**
  - **Cloud Storage:** Saves the Markdown report (`reports/{job_id}/pre_migration_report.md`) and the raw JSON (`reports/{job_id}/inventory.json`) via `src.infrastructure.gcp.services`.
...
- **Class:** `OrchestrationService` (Requires injected `StateInterface`, `StorageInterface`, `TaskQueueInterface`)
...
- **Endpoint:** `POST /api/v1/worker/{stage}` (`src/api/routers/workers.py -> handle_task`)
- **GCP Services:**
...
- **GCP Services:**
  ...
  - **Firestore (Token Bucket):** `ExecutionService._sync_complexity(response)` reads the live `complexity.after` metadata from monday.com and calibrates the bucket.
...
- **GCP Services:**
  ...
  - If `completed_tasks == total_tasks`, the worker calls `OrchestrationService.enqueue_next_stage()`.
  ...
  - **Firestore (Final State):** Once the DAG is fully exhausted, `OrchestrationService` calls `StateManager.update_job_status(job_id, "COMPLETED")` to close out the job and make the final report available.
