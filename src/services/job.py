...
class JobService:
    ...
    def __init__(
        ...
        """
        Initializes the JobService with necessary dependencies and GCP clients.

        # ==============================================================================
        # EDUCATIONAL NOTE: DEPENDENCY INVERSION (SOLID "D")
        # ==============================================================================
        # BEFORE:
        # We had no dependencies injected. JobService imported the concrete
        # ClassificationService directly and instantiated it inline. This violated DIP
        # because the high-level JobService depended on a low-level concrete implementation.
        #
        # POOR MAN's DI (The "or" pattern):
        # We could have done `self.classifier = classifier or ClassificationService()`.
        # But this STILL requires JobService to import `ClassificationService` as the fallback,
        # meaning they are still tightly coupled.
        #
        # AFTER (True Dependency Injection):
        # We inject the Interfaces (Protocols). JobService has ZERO imports of concrete
        # engines now. It is completely decoupled. If we want to unit test JobService,
        # we pass in dummy objects that match the Protocol.
        #
        # THE FACTORY PATTERN (discovery_factory):
        # We can't inject an instantiated DiscoveryService because it requires an API key
        # that we only fetch dynamically during the job execution.
        # Instead, we inject a Factory (`Callable[[str], DiscovererInterface]`). JobService
    ...
    async def execute_discovery_job(self, job_id: str) -> None:
        ...
        try:
            ...
            # discovery_engine = DiscoveryService(client=client)
            #
            # AFTER:
            ...
            # ==============================================================================
            # BEFORE:
            # classification_engine = ClassificationService()
